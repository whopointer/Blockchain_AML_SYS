...
const MoneyLaunderingTrace: React.FC = () => {
  ...
  useEffect(() => {
    ...
    if (task.status === 'completed' || task.status === 'failed' || task.status === 'error') return undefined;
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!startT || !endT) {
        setError('请填写起始时间与终止时间（支持 YYYY-MM-DD，与文档一致按 UTC 日界线）');
        return;
      }
      if (startT > endT) {
      ...
      if (outDegreeLimit !== undefined) cleanedRequest.out_degree_limit = outDegreeLimit;
      ...
      if (activateThreshold !== undefined) cleanedRequest.activate_threshold = activateThreshold;
      ...
      const msg = err.response?.data?.error || err.message || '提交追踪任务失败';
      setError(msg);
    } finally {
      setSubmitting(false);
    }
  ...
  const renderParamLabel = (name: string, description: string, required = false) => (
    <OverlayTrigger
      placement="top"
        ...
        <Row className="g-3 mb-3">
          <Col md={6}>
            <Card>
              ...
              <Card.Body>
                <div><strong>request_id:</strong> {result.request_id}</div>
                <div><strong>status:</strong> {result.status || '-'}</div>
                <div><strong>区块范围:</strong> {result.meta?.start_block_id ?? '-'} - {result.meta?.end_block_id ?? '-'}</div>
                <div><strong>token:</strong> {result.meta?.token || '-'}</div>
                <div><strong>token_address:</strong> {result.meta?.token_address || '-'}</div>
                <div><strong>activate_threshold_usd:</strong> {fmtNumber(result.meta?.activate_threshold_usd)}</div>
                <div><strong>age_limit:</strong> {result.meta?.age_limit ?? '-'}</div>
                <div><strong>execution_time_ms:</strong> {result.meta?.execution_time_ms ?? '-'}</div>
              </Card.Body>
            </Card>
          </Col>
          <Col md={6}>
            <Card>
              ...
              <Card.Body>
                <div><strong>source_address_count:</strong> {result.summary?.source_address_count ?? '-'}</div>
                <div><strong>node_count:</strong> {result.summary?.node_count ?? '-'}</div>
                <div><strong>edge_count:</strong> {result.summary?.edge_count ?? '-'}</div>
                <div><strong>total_flow_usd:</strong> {fmtNumber(result.summary?.total_flow_usd)}</div>
              </Card.Body>
            </Card>
          </Col>
        </Row>
        ...
        <Card className="mb-3">
          ...
          <Card.Body>
            <Row className="g-2 align-items-end mb-3">
              <Col md={3}>
                <Form.Check
                  ...
                  onChange={(e) =>
                    setVizConfig((prev) => ({ ...prev, simplify: e.target.checked }))
                  }
                />
              </Col>
              <Col md={3}>
                <Form.Group>
                  ...
                  <Form.Control
                    ...
                    onChange={(e) =>
                      setVizConfig((prev) => ({ ...prev, min_value: e.target.value }))
                    }
                  />
                </Form.Group>
              </Col>
              <Col md={3}>
                <Form.Group>
                  ...
                  <Form.Control
                    ...
                    onChange={(e) =>
                      setVizConfig((prev) => ({ ...prev, max_nodes: e.target.value }))
                    }
                  />
                </Form.Group>
              </Col>
                ...
                <Form.Group>
                  ...
                  <Form.Select
                    ...
                    onChange={(e) =>
                      setVizConfig((prev) => ({
                        ...prev,
                        layout: e.target.value as 'fdp' | 'dot' | 'neato' | 'sfdp' | 'twopi' | 'circo',
                      }))
                    }
                  >
            ...
            <div className="d-flex gap-2 mb-3">
              <Button onClick={handleGenerateVisualization} disabled={vizLoading}>
                {vizLoading ? (
                  <>
            ...
            {vizError && <Alert variant="danger" className="mb-3">{vizError}</Alert>}

            {svgUrl ? (
              <div style={{ border: '1px solid #dee2e6', borderRadius: 8, overflow: 'hidden' }}>
                {svgContent ? (
                  <div
                    style={{ width: '100%', minHeight: 520, overflow: 'auto', background: '#fff' }}
              ...
              <div className="text-muted">点击“生成图形”后，在此展示资金流向 SVG。</div>
            )}
          </Card.Body>
        </Card>
                ...
                <tbody>
                  {edges.slice(0, 20).map((edge, idx) => (
                    <tr key={`${edge.tx_hash || idx}-${idx}`}>
                      ...
                      <td title={edge.tx_hash}>{edge.tx_hash ? shortAddr(edge.tx_hash) : '-'}</td>
                      <td>{edge.block_id ?? '-'}</td>
        ...
        <Card>
          ...
          <ListGroup variant="flush">
            {Object.entries(files).length === 0 ? (
              <ListGroup.Item className="text-muted">无文件下载信息</ListGroup.Item>
            ) : (
          ...
          <Card>
            <Card.Header className="d-flex justify-content-between align-items-center">
              <div>
                ...
                <small className="text-muted">填写参数后提交，支持一键回填默认配置</small>
              </div>
              <Button
                variant="outline-secondary"
                size="sm"
            ...
            <Card.Body>
              <Form onSubmit={handleSubmit}>
                <Row className="g-3">
                  ...
                  <Col md={6}>
                    <Form.Group>
                      {renderParamLabel('token', '代币类型，支持 USDT 或 ETH', true)}
                      <Form.Select
                  ...
                  <Col md={6}>
                    <Form.Group>
                      ...
                      <Form.Control
                        ...
                        onChange={(e) => onChange('out_degree_limit', e.target.value)}
                        disabled={submitting || polling}
                      />
                    </Form.Group>
                  </Col>
                  <Col md={12}>
                    <Form.Group>
                      {renderParamLabel('src（源地址列表）', '源地址列表', true)}
                      <Form.Control
                        as="textarea"
                        rows={3}
                  ...
                  <Col md={12}>
                    <Form.Group>
                      {renderParamLabel('allowed（可选）', '允许通过的地址列表')}
                      <Form.Control
                        as="textarea"
                        rows={2}
                        placeholder="每行一个地址，或逗号分隔"
                  ...
                  <Col md={12}>
                    <Form.Group>
                      {renderParamLabel('forbidden（可选）', '禁止通过的地址列表')}
                      <Form.Control
                        as="textarea"
                        rows={2}
                        placeholder="每行一个地址，或逗号分隔"
                  ...
                  <Col md={4}>
                    <Form.Group>
                      {renderParamLabel('depth', '搜索深度限制（跳数），从起始节点开始的最大跳数')}
                      <Form.Control
                        type="number"
                  ...
                  <Col md={4}>
                    <Form.Group>
                      {renderParamLabel('activate_threshold', '激活阈值（美元），低于此金额的转账将被忽略')}
                      <Form.Control
                        type="number"
                        step="0.01"
                        value={formData.activate_threshold}
                        onChange={(e) => onChange('activate_threshold', e.target.value)}
                        disabled={submitting || polling}
                      />
                    </Form.Group>
                  </Col>
                  <Col md={4}>
                    <Form.Group>
                      {renderParamLabel('age_limit', '年龄限制（跳数），资金从源地址传播的最大跳数')}
                      <Form.Control
                        type="number"
                        value={formData.age_limit}
                        onChange={(e) => onChange('age_limit', e.target.value)}
                        disabled={submitting || polling}
                      />
                    </Form.Group>
                  </Col>
                  <Col md={12}>
                    <Form.Group>
                      {renderParamLabel('label_limit', '标签数量限制，每个节点最多保留的标签数量')}
                      <Form.Control
                        type="number"
                        value={formData.label_limit}
                        onChange={(e) => onChange('label_limit', e.target.value)}
                        disabled={submitting || polling}
                      />
                    </Form.Group>
                  </Col>
                ...
                <div className="d-grid mt-3">
                  <Button variant="primary" type="submit" disabled={submitting || polling}>
                    {submitting ? (
                      <>
                        <Spinner as="span" animation="border" size="sm" className="me-2" />
                        提交中...
          ...
          {!task && (
            <Card>
              <Card.Body className="text-center py-5">
                ...
                <p className="text-secondary mb-0">任务将异步执行，系统自动轮询状态并展示结果</p>
              </Card.Body>
            </Card>
          )}
          ...
          {task && (
            <Card>
              <Card.Header className="d-flex justify-content-between align-items-center">
                ...
                <Badge bg={getTaskVariant(task.status)}>{task.status}</Badge>
              </Card.Header>
              <Card.Body>
                <Alert variant={getTaskVariant(task.status)} className="mb-3">
                  <div><strong>created_at:</strong> {task.created_at || '-'}</div>
                  <div><strong>started_at:</strong> {task.started_at || '-'}</div>
                  <div><strong>completed_at:</strong> {task.completed_at || '-'}</div>
                  {polling && (
                    <div className="mt-2">
                      <Spinner as="span" animation="border" size="sm" className="me-2" />
                      正在轮询任务状态（每 3 秒）
                ...
                {(task.status === 'failed' || task.status === 'error') && (
                  <Alert variant="danger">{task.error || '任务执行失败'}</Alert>
                )}
